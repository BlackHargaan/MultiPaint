 ...
 *
 * @returns { positions: Float32Array, triGroup: Uint16Array,
 *            filamentColors: string[]|null, dropped: number }
 */
export function parse3MF(arrayBuffer) {
  ...
  const emitObject = (doc, objectId, matrix, path) => {
    ...
    if (mesh) {
      ...
      // whole-part filament assignment (parts-mode round trip)
      ...
      for (const t of mesh.querySelectorAll('triangles > triangle')) {
        ...
        for (const iStr of idx) {
          const p = verts[+iStr];
          positions.push(...applyMatrix(matrix, p));
        }
    ...
    for (const comp of obj.querySelectorAll('components > component')) {
      ...
      if (childDoc) emitObject(childDoc, childId, childMatrix, childPath);
    }
  };

  for (const item of mainDoc.querySelectorAll('build > item')) {
    const matrix = parseTransform(item.getAttribute('transform'));
    emitObject(mainDoc, item.getAttribute('objectid'), matrix, mainPath);
  }
  if (!positions.length) throw new Error('The 3MF contains no printable mesh.');

  // drop degenerate triangles, keeping triGroup aligned
  ...
  return {
    positions: outPos,
    triGroup: outGroup,
    filamentColors: parseFilamentColors(files),
    dropped: triCount - keep.length,
  };
}
